...
let test =
	...
	let to_add : add_param contract = Test.to_entrypoint "add_data"	taddr in
	let () = Test.log("Expected to fail:") in
	let _ = match Test.transfer_to_contract to_add add_param 1tez with
		| Success nat -> true
		| Fail err ->
			begin
				match err with
	...
	let to_remove : sensor_id contract = Test.to_entrypoint "remove_sensor" taddr in
	let () = Test.log("Expected to fail:") in
	let _ = match Test.transfer_to_contract to_remove remove_param 1tez with
		| Success nat -> true
		| Fail err ->
			begin
				match err with
	...
	let to_update : address contract = Test.to_entrypoint "update_admin" taddr in
	let () = Test.log("Expected to fail:") in
	let _ = match Test.transfer_to_contract to_update update_param 1tez with
		| Success nat -> true 
		| Fail err ->
			begin
				match err with
